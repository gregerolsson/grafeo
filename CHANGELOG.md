...
End-to-end tiered storage: section data (LPG, RDF Ring, vector topology) can now spill to mmap-backed disk under memory pressure or explicit configuration, with per-section tier overrides, introspection, and reload. Plus a long list of supporting format work: per-block columnar zone maps, paged HNSW topology, packed RDF Ring on disk, and a WAL overlay for mutating mmap'd compact stores.

### Added

- **`TierOverride` per-section storage configuration**: `Config::with_section_tier(SectionType, TierOverride)` pins individual sections to a tier. `ForceDisk` spills the section to mmap at database open; `ForceRam` pins it in RAM and is hard-enforced (skipped in pressure-driven eviction, in `spill_all`, and in explicit `spill_consumer_by_name`). `Auto` (default) lets the buffer manager decide. Pre-existing `Config::with_section_config` builder accepts a full `SectionMemoryConfig { max_ram, tier }` for combined pinning + RAM cap.
- **`db.storage_tiers()` introspection**: returns a map from section type to current tier (`InMemory` / `OnDisk` / `Uninitialized`). Backed by a new `MemoryConsumer::current_tier` trait method that specialized consumers (`CompactStoreConsumer`, `VectorIndexConsumer`, generic `SectionConsumer`) override with authoritative state. Useful for tests, dashboards, and verifying that a `ForceDisk` config took effect.
- **`db.reload_eligible(target_fraction)`**: brings spilled sections back into RAM after memory pressure drops. Walks consumers reporting `OnDisk` in priority order (highest first), calling `reload()` on each as long as projected usage stays below `target_fraction * memory_limit`. Returns the count of consumers reloaded. Best-effort: per-consumer reload errors are logged and skipped.
- **Python bindings for tier control**: `GrafeoDB(path, section_tiers={'VectorStore': 'force_disk', 'LpgStore': 'force_ram'})`, `db.storage_tiers() -> dict[str, str]`, `db.reload_eligible(target_fraction=0.7) -> int`. Tier values accept both `'force_disk'` and `'ForceDisk'`; invalid section or tier names raise `ValueError` with a clear message. Eight new pytest cases under `crates/bindings/python/tests/test_storage_tiers.py`.
- **Paged HNSW topology format (vector v2)**: `VectorStoreSection` now serializes a packed `GVST` envelope (16-byte header + 32-bytes-per-index directory + per-index `IndexMetaV2` blob + per-index `GTOP` paged topology). Each `GTOP` blob has a sorted page index for `O(log n)` neighbor lookup and a payload region of packed `u64` LE neighbor IDs. New `MmapTopology` reads neighbors directly from `Bytes` slices with zero allocation per query; `HnswIndex` adopts a `TopologyBackend { Heap, Mmap }` enum so all three search paths (greedy, beam, filtered) dispatch through a unified `HnswNeighborsIter`. Heap drops more than 10x when loaded as v2 from mmap; recall is bit-identical between modes (verified across `search`, `search_with_ef`, `search_with_filter`, `search_with_ef_and_filter`, `batch_search`).
- **Packed RDF Ring on-disk format (ring v2)**: replaces bincode `RdfRingSection` with a `GRFR` envelope composed of `PackedTermDictionary` (sorted string table + offsets + sorted-id permutation), three packed wavelet trees (`WTRE` envelope, per-level `Bytes`-backed bitvectors via the new `BitVector::from_mmap`), two packed permutations, and a CRC32 trailer. `RdfRingSection::swap_to_mmap` now wires the format end-to-end: spill writes a v2 buffer, mmap fetcher delivers it back, and the deserializer constructs a Ring whose bulk components share refcounted `Bytes` slices. v1 bincode buffers fall through to the legacy path so existing files keep loading.
- **Per-block columnar zone maps + iterator bounds**: LPG compact-store columns now lay out as 4 KiB blocks with per-block min/max statistics. New `find_in_range_iter` uses zone-map skip for early termination on selective predicates; `RangeScanOperator` streams matching rows directly into the push pipeline. Massive speedups on selective range scans against large columns.
- **WAL overlay for mutable mmap'd LPG**: when the compact base is `OnDisk`, mutations route to a live `LpgStore` overlay; periodic merge under `BufferManager` pressure folds the overlay back into a fresh compact base via `LayeredStore::merge_overlay_in_place`. New `merge_guard: RwLock<()>` makes concurrent reads + writes + merges race-free; new `OverlayConsumer` registers overlay growth with the buffer manager. Persistent `.grafeo` file plus spilled OnDisk base plus overlay mutations now round-trip across process restarts.
- **`SectionType::OverlayDeletions`**: new data-section variant (`= 5`) that persists the `LayeredStore` `deleted_from_base_nodes` / `deleted_from_base_edges` sets so deletions of base entities (nodes or edges that originated in the compact base) survive close/reopen without an explicit `compact()`. New `LayeredStore::seed_deleted_from_base`, `snapshot_deleted_node_ids`, `snapshot_deleted_edge_ids`, `deletions_dirty`, `mark_deletions_clean`. The section is omitted from the container directory entirely when the deletion log is empty, so workloads that don't delete base entities pay nothing. Marked `required: false`: 0.5.41 readers still reject the unknown section type today, but the flag becomes meaningful once the directory parser learns to skip non-required unknowns (planned for 0.5.43).
- **`tracing` events on tier transitions**: with the `tracing` feature enabled, `BufferManager::spill_consumer_by_name`, `BufferManager::reload_eligible`, and `GrafeoDB::apply_force_disk_overrides` emit info events under the `grafeo::buffer` and `grafeo::tier` targets. Wire a `tracing-subscriber` to log spill/reload/ForceDisk activity without needing a metrics framework.
- **Storage Tiers documentation page**: new `docs/architecture/memory/storage-tiers.md` covering the three tiers, default `Auto` policy with pressure thresholds, `ForceDisk` / `ForceRam` overrides, the `with_section_tier` builder, `db.storage_tiers()` introspection, `reload_eligible` semantics, the spill directory, and the `tracing` target conventions.
- **`PageFetcher` trait + `MmapPageFetcher` implementation**: indirection layer between sections and their byte source so the mmap implementation can be swapped for vmcache (TUM, 2024) or an explicit pager later without touching consumers. Lives in `grafeo-common` so both `grafeo-core` (section serializers) and `grafeo-storage` (mmap impl) can reference it without violating the crate-boundary rule.

### Changed

- **Vector store section format bumped to v2** (paged `GVST` envelope). Existing v1 (bincode) files keep loading via magic-byte detection and upgrade to v2 on the next checkpoint.
- **RDF Ring section format bumped to v2** (packed `GRFR` envelope). Existing v1 (bincode) files keep loading via magic-byte detection and upgrade to v2 on the next checkpoint.
- **`TierOverride::ForceDisk` enforcement is now targeted, not global**: previously an `ForceDisk` override on `VectorStore` triggered `spill_all()` for every consumer. The 0.5.42 path uses `spill_consumer_by_name(...)` per matching section, so other consumers stay InMemory.
...
- **`TierOverride::ForceRam` was silently a no-op**: prior versions accepted the config but the buffer manager spilled `ForceRam` consumers anyway under pressure. The spill loop now consults a `force_ram_consumers` set and skips matching consumers in `run_eviction_internal`, `spill_all`, and `spill_consumer_by_name`. Allocations that would have spilled a ForceRam consumer fall through to other consumers or fail.
- **Persistent reopen lost the compact base** when a database had been compacted: directory parser at `grafeo-storage::container::directory.rs` was missing the `4 => SectionType::CompactStore` arm, causing reopen to fall back to a legacy snapshot path with a checksum mismatch; `load_from_sections` had no handler for the `CompactStore` section. Both fixed; new `extract_compact_base` + `wire_layered_after_load` helpers reconstruct the full LayeredStore and consumer wiring on open.
- **Concurrent merge could lose writes**: stress test with 4 readers + writer + merger under `LayeredStore::merge_overlay_in_place` lost 426/500 writes in one run. Fixed via the new `merge_guard: RwLock<()>` (mutators take read, merge takes write).
- **`GRAFEO-X001: snapshot checksum mismatch` opening v2 section files** ([#323](https://github.com/GrafeoDB/grafeo/issues/323), [#324](https://github.com/GrafeoDB/grafeo/pull/324)): `read_snapshot` was unconditionally a v1 reader and would CRC zero bytes against the recorded v2 directory CRC whenever the engine's open path fell through to it (for example after the directory parser silently bailed). The reader now early-returns `Ok(Vec::new())` on a v2 header (`snapshot_length == 0` with non-empty header), letting the engine's v2 dispatch take over. Reported by [@teipsum](https://github.com/teipsum) against an embedded production database.
- **Silent corruption masking in `read_section_directory`** (#323 follow-up): the v2 directory parser swallowed every `SectionDirectory::from_bytes` error with a `_ => Ok(None)` wildcard, routing files with truncated directories or torn-page directory writes into the v1 read logic that surfaced as a misleading snapshot-checksum error. The parser now propagates parse errors with the failing offset, treats "header asserts v2 but file is too short" as an error rather than a v1 fallback, and verifies the directory page CRC against the value the writer recorded in the active header. Two new regression tests cover the parse-error and CRC-mismatch paths.
- **`LayeredStore` deletions silently reverted on reopen** (#323 follow-up): base nodes and edges deleted after `compact()` were tracked only in `deleted_from_base_*` (in-memory). Closing and reopening reconstructed the dirty sets but had no information about deletions, so previously-deleted base entities silently reappeared until the next `compact()` merged them away. The new `OverlayDeletions` section persists the deletion log; the open path reads it via `extract_overlay_deletions` and seeds the layered store via `seed_deleted_from_base` after `with_overlay`. Two new round-trip integration tests (`deleted_base_nodes_stay_deleted_across_reopen`, `deleted_base_edges_stay_deleted_across_reopen`) lock in the behavior.
...
- **`TieredStore` trait** (`grafeo_common::memory::buffer::TieredStore`): never implemented anywhere; the `Section` trait (with `swap_to_mmap` + `reload_to_ram`) plus `MemoryConsumer` cover the same lifecycle. Marked `#[deprecated(since = "0.5.42")]` and scheduled for removal in 0.6.0. The `StorageTier` enum it shipped alongside is kept and remains the standard tier identifier.
