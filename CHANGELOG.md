...
End-to-end tiered storage: section data (LPG, RDF Ring, vector topology) can spill to mmap-backed disk under memory pressure or explicit configuration, with per-section tier overrides, introspection, and reload. Plus per-block columnar zone maps for selective range scans, paged HNSW topology, packed RDF Ring on disk, and a WAL overlay for mutating mmap'd compact stores.
...
- **Paged HNSW topology (vector v2)**: `VectorStoreSection` switches from bincode to a packed `GVST` envelope with per-index `GTOP` paged topology. New `MmapTopology` serves neighbor lookups directly from `Bytes` slices via binary search; `HnswIndex` dispatches all three search paths through a `TopologyBackend { Heap, Mmap }` enum. Heap drops more than 10x when loaded from mmap; recall is bit-identical between modes.
- **Packed RDF Ring (ring v2)**: replaces bincode `RdfRingSection` with a `GRFR` envelope (sorted-id term dictionary + three packed wavelet trees + two packed permutations + CRC32 trailer). `swap_to_mmap` wires the format end-to-end so the Ring's bulk components share refcounted `Bytes` slices from the spill mmap.
- **Per-block columnar zone maps + iterator bounds**: LPG compact-store columns now lay out as 4 KiB blocks with per-block min/max statistics. `find_in_range_iter` uses zone-map skip for early termination; `RangeScanOperator` streams matches into the push pipeline. Selective range scans see large speedups.
- **WAL overlay for mutable mmap'd LPG**: when the compact base is `OnDisk`, mutations route to a live `LpgStore` overlay; periodic merge under buffer-manager pressure folds the overlay back via `LayeredStore::merge_overlay_in_place`. New `merge_guard` makes concurrent reads + writes + merges race-free. Persistent file + spilled base + overlay mutations now round-trip across process restarts.
- **`SectionType::OverlayDeletions`**: new data-section variant (`= 5`) that persists the `LayeredStore` `deleted_from_base_nodes` / `deleted_from_base_edges` sets so deletions of base entities survive close/reopen without an explicit `compact()`. Omitted from the container directory entirely when the deletion log is empty. Marked `required: false`; the flag becomes meaningful once the directory parser learns to skip non-required unknowns (planned for 0.5.43).
...
- **Filter pushdown extended across `Filter`, `LeftJoin`, `Apply`, `Union`, `Unwind`**: predicates inside `OPTIONAL MATCH` and correlated subqueries now reach the scan. A new pre-pushdown pass propagates filter predicates across `LeftJoin` shared-variable boundaries so the right-side subtree of an OPTIONAL MATCH picks up the same `WHERE` constraints as the main `MATCH`. Around 8x speedup on the `feed.hydrate` Cypher pattern; the `Filter(WHERE_pred, Filter(hasLabel, Expand))` shape Cypher emits also no longer gets stuck at the top of the tree, since filter pushdown now commutes through sibling `Filter` operators (gated by a variable-scope check so a predicate over a path variable like `LENGTH(p) >= 1` doesn't commute past a label filter into a subtree where the path isn't yet bound).
...
- **Persistent reopen lost the compact base after `compact()`**: directory parser was missing the `SectionType::CompactStore` arm, and `load_from_sections` had no handler for the section, so reopens fell back to a legacy snapshot path that surfaced as a checksum mismatch. New `extract_compact_base` + `wire_layered_after_load` helpers reconstruct the full LayeredStore and consumer wiring on open.
- **Concurrent merge could lose writes**: stress test with 4 readers + writer + merger under `LayeredStore::merge_overlay_in_place` lost 426/500 writes in one run. Fixed via a new `merge_guard: RwLock<()>` (mutators take read, merge takes write).
